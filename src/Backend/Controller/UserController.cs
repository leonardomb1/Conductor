...
public sealed class UserController(UserRepository repository) : ControllerBase<User>(repository)
{
    public async Task<IResult> Login(Stream body, string ip)
    {
        ...
        if (Settings.AdminLogin == deserialize.Value.Name &&
            Settings.AdminPassword == deserialize.Value.Password)
        {
            string adminJwt = Encryption.GenerateJwt(ip, deserialize.Value.Name, Settings.EncryptionKey);
            return Results.Text(adminJwt, "text/plain");
        }
